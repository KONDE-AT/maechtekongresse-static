...
for file_name in tqdm(file_list, total=len(file_list)):
    ...
    # correspAction/date
    try:
        doc_date_node = doc.any_xpath("//tei:origin//tei:date")[0]
    except IndexError:
        broken.append(f"missing '//tei:origin//tei:date' in file: {file_name}")
        continue
    is_valid_date = False
    if "when-iso" in doc_date_node.attrib:
        ca_date_when = doc_date_node.attrib["when-iso"]
        is_valid_date = True
    elif "when" in doc_date_node.attrib:
        ca_date_when = doc_date_node.attrib["when"]
        is_valid_date = True
    else:

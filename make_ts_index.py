...
try:
    client.collections["maechtekongresse"].delete()
except ObjectNotFound:
    pass

current_schema = {
    "name": "maechtekongresse",
    "fields": [
        {"name": "id", "type": "string"},
...
for x in tqdm(files, total=len(files)):
    ...
    for v in facs:
        ...
        pages += 1
        cfts_record = {
            "project": "maechtekongresse",
        }
        record = {}
        record["id"] = os.path.split(x)[-1].replace(".xml", f".html?tab={str(pages)}")
        cfts_record["id"] = record["id"]
        cfts_record["resolver"] = f"https://github.com/KONDE-AT/maechtekongresse-static/{record['id']}"
...
print(make_index)
print("done with cfts-index maechtekongresse")
